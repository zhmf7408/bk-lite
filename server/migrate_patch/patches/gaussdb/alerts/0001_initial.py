...
# 问题: PostgreSQL 专属索引 GinIndex/BTreeIndex 在 GaussDB ustore 表上不支持
#       GaussDB ustore 不支持 GIN 索引 (gin index is not supported for ustore)
# 处理策略:
#   - BTreeIndex (datetime字段): 因 db_index=True 已创建索引，使用 FakeAddIndex 跳过避免重复
#   - GinIndex (JSONField字段):  使用 FakeAddIndex 跳过（GaussDB ustore 不支持 GIN 索引）
# 受影响索引:
...
class FakeAddIndex(migrations.AddIndex):
    """跳过 JSONField 上的 GinIndex，GaussDB ustore 不支持 GIN 索引"""
    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                # GaussDB 补丁: 直接定义为 JSONField，避免后续 0005 的 AlterField 操作
        ...
        # incident.operator: JSONField 在 GaussDB ustore 不支持 GIN 索引
        FakeAddIndex(
            model_name="incident",
        ...
        # --- 原 GinIndex(JSONField) → FakeAddIndex 跳过 ---
        # event.labels: JSONField 在 GaussDB ustore 不支持 GIN 索引
        FakeAddIndex(
            model_name="event",
        ...
        # --- 原 GinIndex(JSONField) → FakeAddIndex 跳过 ---
        # alert.operator: JSONField 在 GaussDB ustore 不支持 GIN 索引
        FakeAddIndex(
            model_name="alert",
